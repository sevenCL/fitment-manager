...
import com.seven.library.callback.LoadFinishCallBack;
    ...
    private class AutoLoadScrollListener extends OnScrollListener {
        ...
        public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
            ...
            if (getLayoutManager() instanceof LinearLayoutManager) {
                ...
                int[] lastVisibleItem = ((StaggeredGridLayoutManager) getLayoutManager()).findLastVisibleItemPositions(new int[((StaggeredGridLayoutManager) getLayoutManager()).getSpanCount()]);
                int totalItemCount = AutoLoadRecyclerView.this.getAdapter().getItemCount();

                //有回调接口，并且不是加载状态，并且剩下3个item，并且向下滑动，则自动加载
